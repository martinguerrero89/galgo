...
Comb=combineTwoExpressionSet(esets[[2]],esets[[3]])
for(i in 3:length(downl[-which(downl=="GSE39582")])){
...
finalSig= substr(finalSig, 1, nchar(finalSig)-5)
  ...
  SURV=list(CMS=tumortotal1, GALGO=tumortotal2)
  COLS=c(1:8,10)
...
write.xlsx(CentroidsList[[i]], file="CRC_Galgo_Centroids.xlsx", sheetName=i,append=TRUE, row.names=TRUE)
}


write.xlsx(RR_data[,c(6,5,1:4)], file="CRC_CI_Meta.xlsx", sheetName="CI",append=TRUE, row.names=FALSE)
write.xlsx(ccmData, file="CRC_CI_Meta.xlsx", sheetName="Comparison",append=TRUE, row.names=TRUE)
